...
namespace wf14_winforms_thread
{
    public partial class FrmMain : Form
    {
        ...
        {
            BackgroundWorker worker = sender as BackgroundWorker; // 인자값으로 BackgroundWorker 생성 얕은 복사 - Worker랑 주소 참조
        ...
        private long Fibonacci(int arg, BackgroundWorker worker, DoWorkEventArgs e)
        {
            ...
            else
            {
                ...
                // 진행사항을 프로그래스바에 표시
                int percenComplete = (int)(arg / number * 100);
                if (percenComplete > percent)
                {
                    percent = percenComplete;
                    worker.ReportProgress(percenComplete); // ProgressChanged 이벤트 발생
                }
            }
            return result;
        }
